...
interface MessagesAllDao : BaseDao<FirebaseMessagingLocalData> {
    ...
    @Query("SELECT * FROM messages " + "where conversationId = :conversationId ORDER BY createdAt  DESC " + "limit 100 ")
    ...
    @Query("    SELECT * FROM messages " + "where receiverId = :receiverId    ORDER BY createdAt DESC LIMIT 100")
