...
class SharedHelper(private val sharedPreferences: SharedPreferences, private val gson: Gson) {
    ...
    fun <T> addData(key: String, data: T?) {
        ...
        if (data == null) throw RuntimeException("Data can not be null")

        val json = gson.toJson(data)

        if (json != null) putStringData(key, json)
        else throw RuntimeException("Data can not be null")
    }
    ...
    fun <T> addListData(key: String, list: List<T>?) {
        ...
        if (list == null) throw RuntimeException("Data can not be null")
     ...
     * Retrieve list of objects with given key from Shared preferences
     */
    fun <K, T> retrieveHashMapData(key: String, tt: TypeToken<HashMap<K, T>>): HashMap<K, T> {
        ...
        return if (sharedPreferences.contains(key)) {
            ...
            if (json == null) {
                ...
                gson.fromJson<HashMap<K, T>>(json, tt.type)
            }
        } else {
            return HashMap<K, T>()
        }
     ...
     * Add List of object data with given key to Shared preferences
     */
    fun <K, T> addHashMapData(key: String, list: HashMap<K, T>) {
